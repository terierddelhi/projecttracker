...
const SupaService = {
  ...
  // Note: collected_till_date is deliberately NOT accepted here — it's
  // auto-calculated by the payment_schedule trigger (sync_collected_
  // from_payment_schedule). Omitting it from the upsert payload leaves
  // whatever the trigger last computed untouched, rather than
  // overwriting it with a stale manually-typed value.
  upsertFinancials: async function ({ projectId, contractValue, invoicedTillDate }) {
    const { error } = await this.client
      .from('financials')
      .upsert({
  ...
  // -------------------------------------------------------------
  // Payment Tracker — RLS naturally scopes this: admin sees every
  // project's schedule, PI/Co-PI see only their own projects', team
  // members see nothing. Same fetch works for both the Admin Payment
  // Tracker screens and the PI/Co-PI dashboard summary widget.
  // -------------------------------------------------------------
  fetchAllPaymentSchedules: async function () {
    const { data, error } = await this.client
  ...
  fetchProjectPaymentSchedule: async function (projectId) {
    const { data, error } = await this.client
      .from('payment_schedule')
      .select('*')
  ...
  // Admin-only writes — enforced by payment_schedule_write RLS policy,
  // not just by hiding the UI.
  createPaymentScheduleItem: async function ({ projectId, description, dueDate, amount }) {
    const { error } = await this.client
      .from('payment_schedule')
      .insert({
        project_id: projectId, description, due_date: dueDate, amount,
        updated_by: this.currentUser.email
      });
    if (error) throw error;
  },

  updatePaymentScheduleItem: async function (id, fields) {
    const payload = Object.assign({ updated_by: this.currentUser.email, updated_at: new Date().toISOString() }, fields);
    const { error } = await this.client
      .from('payment_schedule')
      .update(payload)
      .eq('id', id);
  ...
  deletePaymentScheduleItem: async function (id) {
    const { error } = await this.client
      .from('payment_schedule')
      .delete()
      .eq('id', id);
